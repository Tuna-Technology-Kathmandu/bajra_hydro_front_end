// import { ReactComponent as Arrow } from '../../../assets/svg/blogArrow.svg';More actions
  ...
  if (isError) {
    return (
      <section className='w-full h-auto p-[75px] max-md:p-[30px]  flex flex-col items-center justify-center'>
        <h2 className='text-xl max-lg:text-lg max-md:text-base max-sm:text-sm font-semibold mb-2'>
        ...
        {isFetching ? (
          ...
          <>
            ...
            <div className='w-full p-5 bg-commonblue text-white absolute z-10 bottom-0 '>
              <p className='lg:text-lg font-semibold tracking-[0.03em] md:text-base text-sm'>
                {data?.messages[0]?.name ?? ''}
              </p>
              <p className='lg:text-[15px] mt-2 font-medium tracking-[0.03em] md:text-sm text-xs'>
            ...
            {data?.messages[0]?.description ? (
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: data.messages[0]?.description,
                }}
              />
            ) : (
